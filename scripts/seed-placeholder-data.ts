 ...
 * Run with: npx ts-node scripts/seed-placeholder-data.ts
 */
...
async function seedApplications(jobRecords: any[]) {
  console.log('📄 Seeding job applications...')

  // Create applications linked to the seeded jobs
  const applications = [
    {
      'Job ID': jobRecords[0].id, // Armed Response Officer
    ...
    {
      'Job ID': jobRecords[1].id, // Security Guard
    ...
    {
      'Job ID': jobRecords[2].id, // CCTV Technician
